...
bool ReButtonClient2::ConnectIoTHubWithDPS(const char* endpoint, const char* scopeId, const char* deviceId, const char* sasKey)
{
	...
	if (GenerateDeviceSasToken(sasKey, deviceId, &deviceSasKeyPtr))
	{
	...
	for (int i = 0; ; i++)
	{
		...
		if (DevkitDPSClientStart(endpoint, scopeId, deviceId, (char*)deviceSasKey.c_str(), NULL, 0, customProvisioningData)) 
		{
	...
	connectionString += deviceSasKey.c_str();

	Serial.println("ActionSendMessagePnP() : Connect IoT Hub");

	ReButton_Property_Pending_Flag = 0;

	if (pnp_device_initialize(connectionString.c_str(), certificates) == 0)
	{
...
void ReButtonClient2::DoWorkPnP(void)
{
